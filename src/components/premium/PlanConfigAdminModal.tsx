...
import { planAdminService, type AdminPlanConfig } from '../../services/planAdminService';

type PlanConfig = AdminPlanConfig;
...
export default function PlanConfigAdminModal({ visible, onClose, token }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const list = await planAdminService.listPlans();
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const data = await planAdminService.getPlan(selectedPlanType);
  ...
  const initializeDefaults = async () => {
    setSaving(true);
    try {
      await planAdminService.initializeDefaults();
  ...
  const createPlan = async () => {
    ...
    setSaving(true);
    try {
      await planAdminService.createPlan(planType);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const data = await planAdminService.updatePlan(config.planType, {
        activo: config.activo,
        graficasAvanzadas: config.graficasAvanzadas,
  ...
  const deletePlan = async () => {
    ...
    setSaving(true);
    try {
      const data = await planAdminService.deletePlan(config.planType);
