...
const RecurrentesList = ({
    ...
    useEffect(() => {
        ...
        const pauseIds = new Set(
            (Array.isArray(dashboardSnapshot.meta?.planEnforcement?.recurrentes?.toPauseOnThisPage)
                ? dashboardSnapshot.meta?.planEnforcement?.recurrentes?.toPauseOnThisPage
                : []
            ).map((x) => String(x))
        );

                const recurrentesSummarySafe = Array.isArray(dashboardSnapshot.recurrentesSummary)
                    ? dashboardSnapshot.recurrentesSummary
                    : [];
                const allBase = recurrentesSummarySafe.map((r) => {
    ...
    const fetchRecurrentes = async (forceFresh = false) => {
        ...
        try {
            ...
            const data = await accountDashboardService.listRecurrentes({
                userId,
                page,
                limit: LIMIT,
            ...
            const items = data.items as Recurrente[];

            const filtrados = esSubcuenta
                ? items.filter((r) => String(r.subcuentaId ?? '') === String(subcuentaId ?? ''))
                : items.filter((r) => {
