...
const BalanceCard: React.FC<BalanceCardProps> = ({ reloadTrigger, onCurrencyChange, dashboardSnapshot }) => {
  ...
  const fetchDatosCuenta = async () => {
    ...
    try {
      ...
      const payload = await accountOperationsService.getCuentaPrincipal();
  ...
  const fetchTransacciones = async () => {
    ...
    try {
      const data = await accountOperationsService.getTransacciones({
        rango: String(periodo),
        withTotals: true,
        ...(monedaActual ? { moneda: monedaActual } : {}),
      });
