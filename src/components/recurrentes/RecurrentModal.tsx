...
import { getCachedActiveSubcuentas } from "../../services/subaccountsCacheService";
...
const RECURRENT_MODAL_DEBUG_PREFIX = "[RecurrentModal]";
...
type SubcuentaOption = {
  id: string;
  nombre: string;
  moneda?: string;
};

function buildSubcuentaOption(item: any, fallbackName = "Subcuenta actual"): SubcuentaOption | null {
  const id = String(
    item?.subcuentaId ??
      item?.subCuentaId ??
      item?.id ??
  ...
  return {
    ...
    nombre: fixEncoding(
      String(
        item?.subcuentaNombre ??
          item?.nombreSubcuenta ??
    ...
    moneda: item?.moneda
      ? String(item.moneda)
      : item?.subcuenta?.moneda
      ? String(item.subcuenta.moneda)
      : undefined,
  };
}
...
const RecurrentModal: React.FC<Props> = ({
  ...
  const [subcuentas, setSubcuentas] = useState<SubcuentaOption[]>([]);
  const [loadingSubcuentas, setLoadingSubcuentas] = useState(false);
  const [selectedSubcuentaId, setSelectedSubcuentaId] = useState<string | null>(subcuentaId || null);
  const [usarSubcuentaPreferida, setUsarSubcuentaPreferida] = useState(false);
  const [resolvedUserId, setResolvedUserId] = useState<string>(String(userId ?? "").trim());
  ...
  const subcuentaTouchedRef = useRef(false);
  ...
  useEffect(() => {
    if (!visible) {
      ...
      subcuentaTouchedRef.current = false;
  ...
  useEffect(() => {
    ...
    const fetchDetalle = async () => {
      ...
      try {
        const data = await subaccountsRecurrentesService.getRecurrenteDetail(editingId);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const cachedSubcuentas = await getCachedActiveSubcuentas(200).catch(() => ({
        items: [],
        isFresh: false,
        updatedAt: null,
      }));
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:start`, {
        visible,
        cuentaId,
        propUserId: userId,
        resolvedUserId,
        subcuentaId,
        isEditing,
        cachedSubcuentasCount: cachedSubcuentas.items.length,
        cachedSubcuentasFresh: cachedSubcuentas.isFresh,
        cachedUpdatedAt: cachedSubcuentas.updatedAt,
      });

      if (cachedSubcuentas.items.length > 0) {
        const cachedOptions = cachedSubcuentas.items
          .filter((item: any) => item && (item.activa ?? item.isActive ?? true))
          .map((item: any) => ({
            id: String(item.subCuentaId ?? item.id ?? item._id ?? "").trim(),
            nombre: fixEncoding(String(item.nombre ?? "Subcuenta")),
            moneda: item.moneda ? String(item.moneda) : undefined,
          }))
          .filter((item: SubcuentaOption) => !!item.id);

        setSubcuentas(cachedOptions);
        console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:cachedSubcuentasApplied`, {
          count: cachedOptions.length,
          ids: cachedOptions.map((item) => item.id),
        });
      }

      const { plataformas, profile, subcuentas: subcuentasArray } =
        await subaccountsRecurrentesService.getRecurrenteBootstrap();
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:bootstrapResponse`, {
        plataformasCount: Array.isArray(plataformas) ? plataformas.length : 0,
        subcuentasCount: Array.isArray(subcuentasArray) ? subcuentasArray.length : 0,
        profileKeys: profile && typeof profile === "object" ? Object.keys(profile).slice(0, 20) : [],
        sampleSubcuenta: subcuentasArray?.[0]
          ? {
              _id: subcuentasArray[0]?._id,
              id: subcuentasArray[0]?.id,
              subCuentaId: subcuentasArray[0]?.subCuentaId,
              nombre: subcuentasArray[0]?.nombre,
              activa: subcuentasArray[0]?.activa,
            }
          : null,
      });

      setPlataformasState(plataformas);

      const nextSubcuentas: SubcuentaOption[] = subcuentasArray
        .filter((item: any) => item && (item.activa ?? item.isActive ?? true))
        .map((item: any) => ({
          id: String(item.subCuentaId ?? item.id ?? item._id ?? "").trim(),
          nombre: fixEncoding(String(item.nombre ?? "Subcuenta")),
      ...
      const currentEditSubcuenta = isEditing
        ? buildSubcuentaOption(recurrenteDetalle ?? recurrenteExistente ?? recurrente)
        : null;
      if (
        currentEditSubcuenta &&
        !nextSubcuentas.some((item) => item.id === currentEditSubcuenta.id)
      ) {
        nextSubcuentas.unshift(currentEditSubcuenta);
      }

      setSubcuentas(nextSubcuentas);
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:normalizedSubcuentas`, {
        count: nextSubcuentas.length,
        ids: nextSubcuentas.map((item) => item.id),
        requestedSubcuentaId: subcuentaId ?? null,
      });

      const prefEnabled = !!profile?.usarSubcuentaPorDefectoEnRecurrentes;
      const prefSubcuentaId = String(
        profile?.subcuentaPorDefectoRecurrentesId ?? "",
      ).trim();
      setUsarSubcuentaPreferida(prefEnabled && !!prefSubcuentaId);
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:profilePreference`, {
        prefEnabled,
        prefSubcuentaId,
        isEditing,
      });

      if (!isEditing && !subcuentaTouchedRef.current) {
        if (subcuentaId) {
          setSelectedSubcuentaId(subcuentaId);
          setAfectaSubcuenta(true);
          setAfectaCuentaPrincipal(false);
          console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:selectedRouteSubcuenta`, {
            selectedSubcuentaId: subcuentaId,
          });
        } else if (prefEnabled && prefSubcuentaId) {
          const exists = nextSubcuentas.some((item: SubcuentaOption) => item.id === prefSubcuentaId);
          if (exists) {
            setSelectedSubcuentaId(prefSubcuentaId);
            setAfectaSubcuenta(true);
            setErrors((prev) =>
              prev.subcuentaId ? { ...prev, subcuentaId: undefined } : prev,
            );
            console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:selectedPreferredSubcuenta`, {
              selectedSubcuentaId: prefSubcuentaId,
            });
          } else {
            console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:preferredSubcuentaMissing`, {
              prefSubcuentaId,
              availableIds: nextSubcuentas.map((item) => item.id),
            });
          }
        }
      }
      if (nextSubcuentas.length === 0) {
        console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:noSubcuentasAvailable`, {
          cachedSubcuentasCount: cachedSubcuentas.items.length,
          prefEnabled,
        });
      }
    } catch (error: any) {
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} fetchData:error`, {
      ...
      Toast.show({
        type: "error",
        text1: fixEncoding("Error al cargar datos"),
      ...
      setLoadingSubcuentas(false);
    }
  }, [visible, recurrenteExistente?.userId, recurrente?.userId, userId, cuentaId, isEditing, subcuentaId, resolvedUserId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const propUserId = String(userId ?? "").trim();
      if (propUserId) {
        console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} resolveUserId:prop`, {
          propUserId,
        });
        if (!cancelled) setResolvedUserId(propUserId);
        return;
      }

      try {
        const snapshot = await getCachedSessionSnapshot();
        const nextUserId = String(snapshot.userId ?? "").trim();
        console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} resolveUserId:snapshot`, {
        ...
        console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} resolveUserId:error`, {
          message: error?.message,
        });
      }
    })();
  ...
  const selectedSubcuenta = useMemo(
    () => subcuentas.find((item) => item.id === selectedSubcuentaId) ?? null,
    [selectedSubcuentaId, subcuentas],
  );
  ...
  const handleAfectaSubcuentaChange = useCallback((value: boolean) => {
    subcuentaTouchedRef.current = true;
    setAfectaSubcuenta(value);
    if (!value) {
      setErrors((prev) =>
        prev.subcuentaId ? { ...prev, subcuentaId: undefined } : prev,
      );
  ...
  const handleSubcuentaSelect = useCallback((nextId: string) => {
    subcuentaTouchedRef.current = true;
    setSelectedSubcuentaId(nextId);
    setAfectaSubcuenta(true);
    setAfectaCuentaPrincipal(false);
    setErrors((prev) =>
      prev.subcuentaId ? { ...prev, subcuentaId: undefined } : prev,
  ...
  const handleAfectaCuentaPrincipalChange = useCallback((value: boolean) => {
    setAfectaCuentaPrincipal(value);
    if (value) {
  ...
  const resetForm = useCallback(() => {
    ...
    setAfectaCuentaPrincipal(!subcuentaId);
    setAfectaSubcuenta(!!subcuentaId);
    setSelectedSubcuentaId(subcuentaId || null);
    setSearch("");
    setErrors({});
    setShowPlatformSearch(false);
  ...
  const handleGuardar = async () => {
    ...
    if (afectaSubcuenta && !selectedSubcuentaId) {
      Toast.show({
        type: "error",
        text1: fixEncoding("Subcuenta requerida"),
        text2: fixEncoding("Selecciona la subcuenta desde donde se descontará este recurrente."),
      });
      setErrors((prev) => ({ ...prev, subcuentaId: fixEncoding("Selecciona una subcuenta") }));
      return;
    }
    ...
    try {
      ...
      if (!isEditing) {
        ...
        const canPerformRes = await planConfigService.canPerform('recurrente', {
      ...
      const normalizedUserId = String(
        resolvedUserId ||
        recurrenteExistente?.userId ||
        recurrente?.userId ||
        userId ||
        "",
      ).trim();

      const normalizedPlataforma = plataforma
        ? {
            ...(String(plataforma?.plataformaId ?? plataforma?.id ?? "").trim()
              ? { plataformaId: String(plataforma?.plataformaId ?? plataforma?.id ?? "").trim() }
              : {}),
            ...(plataforma?.nombre ? { nombre: String(plataforma.nombre) } : {}),
            ...(plataforma?.categoria ? { categoria: String(plataforma.categoria) } : {}),
            ...(plataforma?.color ? { color: String(plataforma.color) } : {}),
          }
        : null;
      ...
      const recurrenteData = {
        ...
        subcuentaId: afectaSubcuenta ? selectedSubcuentaId : null,
        afectaCuentaPrincipal,
        afectaSubcuenta: afectaSubcuenta && !!selectedSubcuentaId,
        recordatorios: recordatoriosSeleccionados,
        tipoRecurrente,
        ...(tipoRecurrente === "plazo_fijo" && { totalPagos }),
      };
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} handleGuardar:payload`, {
        isEditing,
        editingId,
        normalizedUserId,
        recurrenteData,
      ...
      const responseData = await subaccountsRecurrentesService.saveRecurrente(
        recurrenteData,
        isEditing ? recurrenteExistente.recurrenteId : undefined,
      );
      ...
      console.log(`${RECURRENT_MODAL_DEBUG_PREFIX} handleGuardar:error`, {
        isEditing,
        editingId,
        message: error?.message,
        status: error?.statusCode ?? error?.status,
      ...
      Toast.show({
        ...
        text2: fixEncoding(
          error?.message ||
            `No se pudo ${isEditing ? "actualizar" : "guardar"} el recurrente. Intenta de nuevo.`,
        ),
      });
    } finally {
      setSaving(false);
  ...
  useEffect(() => {
    ...
    setSelectedSubcuentaId(
      source.subcuentaId ? String(source.subcuentaId) : subcuentaId || null,
    );
    const sourceSubcuenta = buildSubcuentaOption(source);
    if (sourceSubcuenta) {
      setSubcuentas((prev) => {
        if (prev.some((item) => item.id === sourceSubcuenta.id)) return prev;
        return [sourceSubcuenta, ...prev];
      });
    }
  ...
  useEffect(() => {
    if (!visible || !recurrenteDetalle || subcuentaTouchedRef.current) return;

    const detailSubcuenta = buildSubcuentaOption(recurrenteDetalle);
    const detailSubcuentaId = detailSubcuenta?.id ?? null;

    if (detailSubcuentaId) {
      setSelectedSubcuentaId(detailSubcuentaId);
    }
    if (detailSubcuenta) {
      setSubcuentas((prev) => {
        if (prev.some((item) => item.id === detailSubcuenta.id)) return prev;
        return [detailSubcuenta, ...prev];
      });
    }
    if (typeof recurrenteDetalle?.afectaSubcuenta === "boolean") {
      setAfectaSubcuenta(recurrenteDetalle.afectaSubcuenta);
    }
    if (typeof recurrenteDetalle?.afectaCuentaPrincipal === "boolean") {
      setAfectaCuentaPrincipal(recurrenteDetalle.afectaCuentaPrincipal);
    }
  }, [visible, recurrenteDetalle]);
                ...
                <View style={styles.fieldContainer}>
                  ...
                  >
                    ...
                    <Switch
                      ...
                      onValueChange={handleAfectaCuentaPrincipalChange}
                      trackColor={{
                        false: colors.border,
                        true: colors.button,
                      }}
                  ...
                  >
                    <View style={styles.switchContent}>
                      ...
                      <View style={styles.switchTextContainer}>
                        ...
                        >
                          {fixEncoding("Asigna una subcuenta si quieres que este cobro se descuente desde un saldo separado.")}
                        </Text>
                      </View>
                    </View>
                    <Switch
                      value={afectaSubcuenta}
                      onValueChange={handleAfectaSubcuentaChange}
                      trackColor={{
                        false: colors.border,
                        true: colors.button,
                      }}
                  ...
                  {afectaSubcuenta && (
                    <View
                      style={[
                        styles.subcuentaSelectorCard,
                        {
                          backgroundColor: colors.cardSecondary,
                          borderColor: errors.subcuentaId ? "#EF4444" : colors.border,
                        },
                      ]}
                    >
                      <Text style={[styles.subcuentaSelectorTitle, { color: colors.text }]}>
                        {fixEncoding("Subcuenta desde donde se cobrará")}
                      </Text>
                      <Text style={[styles.switchDesc, { color: colors.textSecondary }]}>
                        {fixEncoding("La subcuenta asignada será la fuente del dinero cuando este recurrente se cobre. También podrás cambiarla después desde el detalle del recurrente.")}
                      </Text>

                      {usarSubcuentaPreferida && !isEditing && selectedSubcuentaId ? (
                        <View
                          style={[
                            styles.infoBox,
                            {
                        ...
                        >
                          ...
                          >
                            {fixEncoding("Se preseleccionó tu subcuenta preferida para recurrentes.")}
                          </Text>
                        </View>
                      ) : null}

                      <View style={styles.subcuentaOptionsList}>
                        {loadingSubcuentas ? (
                          <Text style={[styles.helpText, { color: colors.textSecondary }]}>
                            {fixEncoding("Cargando subcuentas activas...")}
                          </Text>
                        ) : subcuentas.length > 0 ? (
                          subcuentas.map((item) => {
                            const isSelected = item.id === selectedSubcuenta?.id;
                            return (
                              <TouchableOpacity
                                key={item.id}
                                style={[
                                  styles.subcuentaOption,
                                  {
                                    backgroundColor: isSelected ? colors.button + "14" : colors.inputBackground,
                                    borderColor: isSelected ? colors.button : colors.border,
                                  },
                                ]}
                                activeOpacity={0.85}
                                onPress={() => handleSubcuentaSelect(item.id)}
                              >
                                <View style={styles.subcuentaOptionCopy}>
                                  <Text
                                    style={[
                                      styles.subcuentaOptionTitle,
                                      { color: isSelected ? colors.button : colors.text },
                                    ]}
                                  >
                                    {item.nombre}
                                  </Text>
                                  <Text
                                    style={[
                                      styles.subcuentaOptionSubtitle,
                                      { color: colors.textSecondary },
                                    ]}
                                  >
                                    {item.moneda ? `Subcuenta activa · ${item.moneda}` : "Subcuenta activa"}
                                  </Text>
                                </View>
                                <Ionicons
                                  name={isSelected ? "checkmark-circle" : "ellipse-outline"}
                                  size={20}
                                  color={isSelected ? colors.button : colors.textSecondary}
                                />
                              </TouchableOpacity>
                            );
                          })
                          ...
                          <Text style={[styles.helpText, { color: colors.textSecondary }]}>
                            {fixEncoding("No hay subcuentas activas disponibles para asignar.")}
                          </Text>
                        )}
                      </View>
                      {renderError(errors.subcuentaId)}
                    </View>
                  )}
                </View>
...
const styles = StyleSheet.create({
  ...
  subcuentaSelectorCard: {
    borderWidth: 1,
    borderRadius: 14,
    padding: 14,
    marginTop: 10,
  },
  subcuentaSelectorTitle: {
    fontSize: 14,
    fontWeight: "600",
    marginBottom: 6,
  },
  subcuentaOptionsList: {
    gap: 10,
    marginTop: 12,
  },
  subcuentaOption: {
  ...
  subcuentaOptionTitle: {
    fontSize: 14,
    fontWeight: "600",
  },
  subcuentaOptionSubtitle: {
    fontSize: 12,
    marginTop: 2,
  },
  infoBox: {
