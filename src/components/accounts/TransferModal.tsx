...
import {
  ...
  type SubcuentaLite,
} from '../../services/accountDashboardService';
...
const TransferModal: React.FC<Props> = ({
  ...
  const fetchBootstrap = useCallback(async () => {
    try {
      ...
      const [cuentaRaw, subcuentasArray] = await Promise.all([
        accountDashboardService.getCuentaPrincipal(),
        accountDashboardService.getSubcuentasActivas(),
      ]);
      ...
      const subcuentasData: SubcuentaResponse[] = Array.isArray(subcuentasArray)
        ? (subcuentasArray as SubcuentaLite[])
        : [];

      const subcuentasOptions: Option[] = subcuentasData
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      setSubmitting(true);
      const body = await accountDashboardService.createTransfer(payload);

      Toast.show({
        type: 'success',
        text1: (body as any)?.message || 'Transferencia procesada',
