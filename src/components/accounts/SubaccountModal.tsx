...
const SubaccountModal: React.FC<Props> = ({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const responseData = await subaccountsRecurrentesService.upsertSubcuenta(
        payload,
        subcuentaToEdit?.id,
      );
      const msg = responseData?.message || "Error desconocido al guardar subcuenta";
      if (typeof msg === 'string' && msg.toLowerCase().includes('saldo insuficiente')) {
      ...
      logger.error("Error en handleCreate de SubaccountModal", {
        message: err?.message,
      });
      Toast.show({
        type: "error",
