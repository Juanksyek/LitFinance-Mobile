...
const SubaccountsList: React.FC<Props> = ({ userId, refreshKey = 0, dashboardSnapshot }) => {
  ...
  const fetchExternalRates = async (targetCurrency: string): Promise<{ rates: Record<string, number>; baseCurrency?: string }> => {
    ...
    try {
        const body = await financeService.getMonedas();
        ...
        for (const item of Array.isArray(body) ? body : []) {
          ...
          const code = raw.codigo || raw.currency || raw.code || raw.moneda || raw.from || raw.to;
          const rate = raw.tasaBase || raw.tasa || raw.rate || raw.value || raw.ratio || raw.price;
          if (raw.esPrincipal || raw.isPrincipal) detectedBase = String(code).toUpperCase();
  ...
  useEffect(() => {
    ...
    const pauseIds = new Set(
      (Array.isArray(dashboardSnapshot.meta?.planEnforcement?.subcuentas?.toPauseOnThisPage)
        ? dashboardSnapshot.meta?.planEnforcement?.subcuentas?.toPauseOnThisPage
        : []
      ).map((x) => String(x))
    );

    const subaccountsSummarySafe = Array.isArray(dashboardSnapshot.subaccountsSummary)
      ? dashboardSnapshot.subaccountsSummary
      : [];
    const all: Subcuenta[] = subaccountsSummarySafe.map((s) => ({
  ...
  const fetchSubcuentas = async (forceFresh = false) => {
    ...
    try {
      ...
      const data = await accountDashboardService.listSubcuentas({
      ...
      const rawData: Subcuenta[] = items.map((item: any) => ({
        ...
        cantidad: Number(item?.cantidad ?? item?.saldo ?? item?.balance ?? 0),
        moneda: String(item?.moneda ?? 'MXN'),
        simbolo: String(item?.simbolo ?? ''),
        color: String(item?.color ?? '#4CAF50'),
        afectaCuenta: Boolean(item?.afectaCuenta ?? true),
        updatedAt: String(item?.updatedAt ?? ''),
        createdAt: item?.createdAt ? String(item.createdAt) : undefined,
        activa: Boolean(item?.activa),
        origenSaldo: item?.origenSaldo === 'cuenta_principal' ? 'cuenta_principal' : (item?.origenSaldo === 'nuevo' ? 'nuevo' : undefined),
        pausadaPorPlan: Boolean(item?.pausadaPorPlan),
      }));
