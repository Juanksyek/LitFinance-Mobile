...
type SubaccountMovementAppliedPayload = {
  subcuentaId: string;
  tipo: 'ingreso' | 'egreso';
  monto: number;
  ...
  const fetchCuentaYConceptos = useCallback(async () => {
    try {
      ...
      const [resCuenta, resConceptos] = await Promise.all([
        accountOperationsService.getCuentaPrincipal(),
        accountOperationsService.getConceptos(),
      ]);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!conceptoFinal) {
      ...
      try {
        if (afectaCuenta) {
          if (isSubcuenta && subcuentaId) {
            const dataBal = await accountOperationsService.getSubcuentaDetalle(subcuentaId);
            currentBalance = Number(dataBal?.cantidad ?? dataBal?.saldo ?? dataBal?.balance ?? null);
          } else {
            const payloadBal = await accountOperationsService.getCuentaPrincipal();
      ...
      if (!isConnected) {
        ...
        if (isSubcuenta && subcuentaId && afectaCuenta) {
          EventBus.emit('subaccountMovementApplied', {
            subcuentaId,
            tipo,
            monto: montoNumerico,
            afectaCuenta,
          } satisfies SubaccountMovementAppliedPayload);
        }
        Toast.show({
          type: 'success',
      ...
      let res: any;
      try {
        res = await accountOperationsService.createMovimiento(payload);
      } catch (e: any) {
